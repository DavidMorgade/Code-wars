...
export function nbMonths(startPriceOld: number, startPriceNew: number, savingPerMonth: number, percentLossByMonth: number): number[] {
    ...
    let oldCar: number = startPriceOld;
    let newCar: number = startPriceNew;
    let percentLoss: number = percentLossByMonth;
    let savingsPerMonth: number = savingPerMonth;
    ...
    while (savings + oldCar < newCar) {
        ...
        oldCar -= oldCar * (percentLoss / 100);
        newCar -= newCar * (percentLoss / 100);
        savings += savingsPerMonth;
    }
    return [months, Math.round(savings + oldCar - newCar)];
}